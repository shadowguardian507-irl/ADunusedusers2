...
class TestMstimestampparser extends TestCase
{
    ...
    public function testComponentFileisPHPFile()
    {
        ...
        if( strpos(file_get_contents(dirname(__FILE__) .'/../../../scriptsrc/components/php/mstimestampparser.enabled.comp.php'),"<?php") !== false) {
        }
        else
        {
            $isPHPFile=false;
        }
        if( strpos(file_get_contents(dirname(__FILE__) .'/../../../scriptsrc/components/php/mstimestampparser.enabled.comp.php'),"?>") !== false) {
        }
        else
        {
            $isPHPFile=false;
